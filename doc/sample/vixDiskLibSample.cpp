...
void VixDiskAsyncReadWrite(VixDisk::Ptr disk, bool read)
{
   ...
   PrintStat(read, total, end, appGlobals.bufSize * maxOps, prefix.str().c_str());
}

void VixDiskReadWrite(VixDisk::Ptr disk, bool read)
{
   size_t bufSize;
   uint8 *buf;
   ...
   for (i = 0; i < maxOps; i++) {
      ...
      if (bufUpdate >= BUFS_PER_STAT) {
         gettimeofday(&end, NULL);
         PrintStat(read, start, end, bufUpdate, prefix.str().c_str());
         start = end;
         bufUpdate = 0;
      }
   }
...
DoRWBench(bool read) // IN
{
   ...
   for (int i = 0 ; i < appGlobals.diskPaths.size() ; ++i) {
      VixDisk::Ptr disk = boost::make_shared<VixDisk>(appGlobals.connection,
                   ...
                   appGlobals.openFlags, i);
      disks[i] = disk;
      tasks.addTask(boost::bind(&VixDiskReadWrite, disk, read));
   }
}
