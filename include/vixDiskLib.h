...
// flags for NBD compression algorithm (6 bits reserved)
#define VIXDISKLIB_FLAG_OPEN_COMPRESSION_ZLIB    (1 << 4) // open for nbd with compression by zlib
#define VIXDISKLIB_FLAG_OPEN_COMPRESSION_FASTLZ  (1 << 5) // open for nbd with compression by fastlz
#define VIXDISKLIB_FLAG_OPEN_COMPRESSION_SKIPZ   (1 << 6) // open for nbd with compression by skipz
#define VIXDISKLIB_FLAG_OPEN_COMPRESSION_MASK (0x03f0) // mask for compression algorithms
 ...
 *
 * @param connectParams [in] Hostname and login credentials to connect to
...
/**
 * Flush the async write data to disk
 * @param diskHandle [in] Handle to an open virtual disk.
 * @return VIX_OK if success, suitable VIX error code otherwise.
 */
VixError
